...
{
    PERCFG |= BV(0);     /* Set UART0 I/O to Alt. 2 location on P1 */
    ...
    P2SEL &= ~BV(6);        //UART0 first in P1

    /* Setup GPIO for interrupts by falling edge on SPI_RDY_IN */
    P1IEN |= BV(4);       //P1_4
    PICTL |= BV(2);       //Falling edge ISR on P1.4-7 pins
...
static void HalUARTPollSPI(void)
{
...
#endif
  {
    ...
    spiCB(HAL_UART_PORT_0, HAL_UART_TX_EMPTY);  //·¢ËÍÍê³É
    
    spiParseRx();       //ÄÚ²¿Ñ­»·ÒÀ´Î´¦Àí½ÓÊÕµÄÊý¾Ý
  }
  ...
  {    //Ã»ÊÕÈ«
    if (spiCB != NULL)
    {
      ...
      spiCB(HAL_UART_PORT_0, HAL_UART_RX_TIMEOUT);
    }
  }
...
static void spiParseRx(void)
{
  while (1)
  {
    ...
    {   //»¹ÓÐÃ»ÓÐ¸²¸ÇµÄÊý¾Ý£¨Ã»ÓÐÊÕÈ«£©
#if defined HAL_SPI_MASTER
      if (SPI_RDY_IN() && (spiTxLen == 0))
      {
        SPI_CLOCK_RX(1);        //ÔÙ¶ÁÈ¡Ò»¸ö×Ö½Ú
        continue;
      }
#endif
      break;
    ...
    SPI_CLR_RX_BYTE(spiRxIdx);  //±êÊ¶Îª¶Á¹ý
